...
def test_resolve_and_link_maps_arxiv_manifestation_to_canonical_work():
    citing = _paper("arxiv:2507.01485", "BioMARS", source="arxiv")
    target = _paper("openalex:W4412759774", "CRISPR-GPT for agentic automation")
    target.manifestations = [{"id": "arxiv:2404.18021", "source": "arxiv",
                              "url": "https://arxiv.org/abs/2404.18021"}]
    atlas = _atlas(citing, target)
    resolver = _resolver({"2404.18021": _resolved(arxiv_id="2404.18021",
                                                   method="arxiv", score=1.0)})
    n = citelinks.resolve_and_link(
        atlas, extra_refs={citing.id: ["Huang et al. CRISPR-GPT. arXiv:2404.18021."]},
        resolver=resolver, progress=lambda *_: None)
    assert n == 1
    assert any((e.src, e.dst) == (citing.id, target.id) for e in atlas.edges)


def test_reconcile_incoming_maps_version_to_corpus(monkeypatch):
    citing = _paper("openalex:W2", "A Citing Work With A Publisher Title")
    target = _paper("openalex:W1", "The Target Work")
    returned = _paper("arxiv:2401.00002", "A Citing Work With A Publisher Title", "arxiv")
    monkeypatch.setattr(citelinks.openalex, "cited_by",
                        lambda oid, max_results=500: [returned] if oid == target.id else [])
    monkeypatch.setattr(citelinks.semanticscholar, "citations", lambda *_a, **_k: [])
    atlas = _atlas(citing, target)
    assert citelinks.reconcile_incoming(atlas, progress=lambda *_: None) == 1
    assert any((e.src, e.dst, e.evidence) ==
               (citing.id, target.id, "incoming:openalex") for e in atlas.edges)
