...
def main() -> None:
    ...
    ap.add_argument("--output", default="citations_fulltext_complete.json",
                    help="output filename under experiments/edge_quality/out")
    args = ap.parse_args()

    papers = [json.loads(l) for l in open(args.papers) if l.strip()]
    ft_dir = Path(args.fulltext)

    # Map each corpus paper -> its cached fulltext. The original experiment only
    # looked for arxiv_<id> files, silently excluding OpenAlex-named caches even
    # when their full text was present. Prefer the canonical paper-id filename,
    # then retain the arXiv fallback for older/versioned caches.
    files = {f.name: f for f in ft_dir.glob("*.txt")}
    def ft_of(p: dict) -> Path | None:
        canonical = re.sub(r"[^A-Za-z0-9._-]", "_", p["id"]) + ".txt"
    ...
    for p in papers:
        ...
        for q in papers:
            ...
            if s["arx"] and re.search(rf"(?<![\d.]){re.escape(s['arx'])}(?![\d])", text):
                ...
                if s["arx"] != sig[p["id"]]["arx"]:
                    matched.append("arxiv_id")
            if s["title"] and s["title"] in tnorm:
                # exclude self-title (it's obviously in its own text)
                if s["title"] != sig[p["id"]]["title"]:
                    matched.append("exact_normalized_title")
            if matched:
                edge = (p["id"], q["id"])
                edges.add(edge)          # p's text mentions q => p cites q
                evidence[f"{edge[0]}->{edge[1]}"] = matched
    ...
    print(f"scanned {n_scanned}/{len(papers)} fulltexts -> {len(edges)} intra-corpus "
          f"citation edges -> {output}", flush=True)
