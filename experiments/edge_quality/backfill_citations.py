...
def main() -> None:
    ...
    for i, p in enumerate(papers):
        ...
        if not sid:
            ...
            save(); continue
        ext = [{"doi": norm_doi(e.get("doi")), "arx": e.get("arx")}
               for e in semanticscholar.reference_external_ids(sid, max_results=1000)]
        refs[pid] = {"source": "s2" if ext else "none", "oa_refs": [], "ext": ext}
        save()
