...
class CorpusIndex:
    ...
    def from_papers(cls, papers: Iterable["Paper"]) -> "CorpusIndex":
        idx = cls()
        for p in papers:
            # Index every manifestation under the canonical work id. Otherwise a
            # bibliography citing an arXiv preprint cannot join to the retained
            # OpenAlex/publisher node for the same work.
            if hasattr(p, "all_manifestations"):
                records = p.all_manifestations()
            elif hasattr(p, "to_dict"):
                records = [p.to_dict()]
            else:
                # Keep the index useful for lightweight Paper-like records used
                # by callers and tests; manifestations are an enrichment, not a
                # new requirement on the public boundary.
                records = [{"id": getattr(p, "id", ""),
            ...
            for record in records:
                ...
                haystack = " ".join(str(record.get(k) or "") for k in
                                    ("id", "url", "pdf_url", "doi"))
                match = re.search(r"(?:arxiv[:./]|abs/|pdf/)(\d{4}\.\d{4,5}(?:v\d+)?)",
                                  haystack, re.I)
                stem = _norm_arxiv(match.group(1)) if match else (
                    _norm_arxiv(rid[6:]) if rid.startswith("arxiv:") else "")
                if stem:
                    idx.by_arxiv.setdefault(stem, p.id)
                d = _norm_doi(record.get("doi"))
                if d:
                    idx.by_doi.setdefault(d, p.id)
                t = _norm_title(record.get("title") or "")
                if len(t) >= 8:
