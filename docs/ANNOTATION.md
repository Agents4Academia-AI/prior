...
Lightweight **username + password** (no real accounts) — enough for a demo you hand to
others, with independent per-person annotations.

- `data/users.json` maps `name -> {password, admin}` (copy `data/users.json.example`).
  **Real tokens are gitignored.**
- If `data/users.json` is absent, auth runs in **open dev mode**: any name, no password,
  non-admin (handy locally).
- The browser sends `X-Prior-User` / `X-Prior-Password` headers (set via the sign-in box).
...
2. `prior serve` + the UI; click **sign in** (name + password) in the sidebar.
...
```
All take the `X-Prior-User`/`X-Prior-Password` headers. The graph endpoints
